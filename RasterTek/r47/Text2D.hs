...
data Text2DExt = Text [GLfloat] [GLfloat] Text2D
...
instance Render Text2DExt where
    render arg@(Text world ortho (Text2D _ (Just shader) _ baseview s1 s2)) = do
        ...
        maybe
            ...
            (\font -> do
                ...
                return . (,) True $ text2D {
                    ...
                    getSentence2 = s2 })
            (getFont text2D)
    update text2D (IF fps _) = do
        let (width,height) = getScreenSize text2D
        
        maybe
            (return (False,text2D))
            (\font -> do
                let [(c1,d1),(c2,d2)] = sentenceData width height ("FPS: " ++ show fps) 20 20 "CPU: n/a" 20 40 font
                s1 <- updateSentence (getSentence1 text2D) d1 c1 (case (fps >= 60, fps < 30) of
                    (True,_) -> (0,1,0)
                    (_,True) -> (1,0,0)
                    _ -> (1,1,0))
                s2 <- updateSentence (getSentence2 text2D) d2 c2 (0,0,0)
                
                return . (,) True $ text2D {
                    getSentence1 = s1 ,
                    getSentence2 = s2 })
            (getFont text2D)
    update text2D (I count) = do
        let (width,height) = getScreenSize text2D
        
        maybe
            (return (False,text2D))
            (\font -> do
                let [(c1,d1),(c2,d2)] = sentenceData width height ("Render Count: " ++ show count) 20 20 "" 20 40 font
                s1 <- updateSentence (getSentence1 text2D) d1 c1 (1,1,1)
                s2 <- updateSentence (getSentence2 text2D) d2 c2 (1,1,1)
                ...
                return . (,) True $ text2D {
                    ...
                    getSentence2 = s2 })
            (getFont text2D)
    update text2D (B intersect) = do
        let (width,height) = getScreenSize text2D
        
        maybe
            (return (False,text2D))
            (\font -> do
                let [(c1,d1),(c2,d2)] = sentenceData width height ("Intersection: " ++ if intersect then "Yes" else "No") 20 20 "" 20 40 font
                s1 <- updateSentence (getSentence1 text2D) d1 c1 (if intersect then (0,1,0) else (1,0,0))
                s2 <- updateSentence (getSentence2 text2D) d2 c2 (1,1,1)
