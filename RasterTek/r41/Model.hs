module Model (Model,Model.initialize,getPosition,modelTexture) where
...
initialize dataFile texFile texUnit wrap = do
    ...
    withArray vertices $ \ptr ->
        glBufferData GL_ARRAY_BUFFER (fromIntegral $ numVertices*vertexSize) ptr GL_DYNAMIC_DRAW
...
instance Shutdown Model where
    shutdown (Model _ vArray vBuffer iBuffer _ texture) = do
        shutdown texture
...
modelTexture (Model _ _ _ _ _ (Just texture)) = fromIntegral . textureUnit $ texture
