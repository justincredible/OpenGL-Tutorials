module ParticleShader (ParticleShader,initialize,parameters) where
...
data ParticleShader = ParticleShader {
    getProgram :: GLuint,
    getShaders :: [GLuint],
...
initialize = do
    (success, program, shaders) <- compileAndLink ["glsl/particle.vert","glsl/particle.frag"]
    ...
    then return (False, Nothing)
        ...
        return (success, Just $ ParticleShader
            program shaders world view projection texlocn)

instance Shutdown ParticleShader where
    shutdown (ParticleShader program shaders _ _ _ _) = do
...
parameters (ParticleShader program _ world view projection texlocn)
