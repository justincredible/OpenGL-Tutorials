...
fn main() {
    ...
    let shader = Program::new(Rc::clone(&gl)).link("src/5.3.parallax_mapping.vs", "src/5.3.parallax_mapping.fs");

    let quad = VertexArray::new_bump_quad(Rc::clone(&gl));

    let diffuse = Texture::new(Rc::clone(&gl), "", "").load("resources/textures/bricks2.jpg", false);
    let normal = Texture::new(Rc::clone(&gl), "", "").load("resources/textures/bricks2_normal.jpg", false);
    let height = Texture::new(Rc::clone(&gl), "", "").load("resources/textures/bricks2_disp.jpg", false);
    /*
    let diffuse = Texture::new(Rc::clone(&gl), "", "").load("resources/textures/toy_box_diffuse.png", false);
    let normal = Texture::new(Rc::clone(&gl), "", "").load("resources/textures/toy_box_normal.png", false);
    let height = Texture::new(Rc::clone(&gl), "", "").load("resources/textures/toy_box_disp.png", false);
    */
    ...
    while !window.should_close() {
        ...
        process_input(&mut camera, &mut height_scale, &mut window, delta_time);
        ...
        height.bind_active(gl::TEXTURE2);
        quad.bind();
        quad.draw();
...
fn process_input(camera: &mut Camera, height_scale: &mut f32, window: &mut Window, delta_time: f32) {
