...
data Frame = Frame {
    ...
    getShader :: Maybe TranslateShader,
    getTranslation :: GLfloat }
    deriving (Eq, Show)
...
initialize window width height = do
    ...
    (shaderscs,shader) <- TranslateShader.initialize
    return (modelscs && shaderscs, Just $
        Frame window opengl camera model shader 0)
...
instance Render Frame where
    render frame = do
        ...
        let Just model = getModel frame
            ...
            translate = getTranslation frame + 0.01
        ...
        return . (,) True $ frame {
            getCamera = camera,
            getTranslation = if translate > 1 then translate - 1 else translate }
