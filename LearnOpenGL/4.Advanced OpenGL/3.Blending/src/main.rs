...
fn main() {
    ...
    let shader = Program::new(Rc::clone(&gl)).link("src/3.2.blending.vs", "src/3.2.blending.fs");
    ...
    let transparent = VertexArray::new_shape(Rc::clone(&gl), Vec::from(&TRANSPARENT_VERTICES[..]), Vec::from(&INDICES[..6]));
    ...
    let transparent_tex = Texture::new(Rc::clone(&gl), "", "").load("resources/textures/window.png");

    let mut windows: [Vec3; 5] = [
    ...
    while !window.should_close() {
        ...
        windows.sort_by(|v, t| {
            ...
            (p - v).magnitude().partial_cmp(&(p - t).magnitude()).unwrap()
        });
        ...
        transparent.bind();
        transparent_tex.bind_active(gl::TEXTURE0);
        for window in windows.into_iter().rev() {
            shader.set_mat4("model", Mat4::translate(window));
            transparent.draw();
        }
...
const TRANSPARENT_COUNT: usize = 4;
const TRANSPARENT_COMPONENTS: usize = CUBE_COMPONENTS;
const TRANSPARENT_VERTICES: [f32; TRANSPARENT_COMPONENTS * TRANSPARENT_COUNT] = [
    // inverted y coordinates because texture is flipped upside down
    0.0, -0.5, 0.0, 0.0, 1.0, 1.0, -0.5, 0.0, 1.0, 1.0, 0.0, 0.5, 0.0, 0.0, 0.0, 1.0, 0.5, 0.0, 1.0, 0.0,
];
