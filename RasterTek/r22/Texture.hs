module Texture (Texture,initialize,textureUnit) where
...
initialize texFile texUnit wrap = do
    ...
    if bpp /= 32
    then do
        putStrLn "Texture requires 32 bits per pixel."
        ...
        withArray image $
            glTexImage2D GL_TEXTURE_2D 0 (fromIntegral GL_RGBA) (fromIntegral width) (fromIntegral height) 0 GL_BGRA GL_UNSIGNED_BYTE
        ...
        return . (,) True . Just $ Texture texID texUnit
    where
    horner w i = i*2^finiteBitSize w + fromIntegral w

instance Shutdown Texture where
    shutdown texture = with (getID texture) $ glDeleteTextures 1
