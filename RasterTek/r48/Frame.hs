...
initialize window width height = do
    ...
    (_,light) <- ShadowLight.initialize
        >>= flip update (F4 20 20 shadowNear shadowFar)
    ...
    return (and [cubescs,spherescs,groundscs,rentexscs,dscs,sscs], Just $
    ...
    update frame (LF2F position rotation frametime) = do
        ...
        return (True, frame {
            ...
            getLight = (getLight frame) {
                ...
                ShadowLight.getPosition = [lightposx,8,-0.1],
...
instance Render SubRender where
    render (Sub ToTexture frame) = do
        ...
        let Just cube = getCube frame
            ...
            Just shader = getDepthShader frame
