...
run = do
    ...
    window <- openWindow "Tutorial 16" width height
    ...
    then closeWindow frame window
        ...
        engine <- loop $ Engine window frame input timer (Position 0 0 0 0)
    ...
    loop engine = do
        ...
        let position = turn (leftDown input) (rightDown input) (getPosition engine) { Position.getFrameTime = Timer.getFrameTime timer }
        
        (success, frame) <- update (getFrame engine) (F $ getRotationY position)
            >>= (render.snd)
