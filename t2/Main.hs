...
main = do
    ...
    let width = 800
        height = 600
    window <- openWindow "Tutorial 2" width height
    ...
    loop window shader vertexArray
    ...
    terminate
    where
    loop window shader vertexArray = do
        ...
        when (escape /= KeyState'Pressed && not quit) $ loop window shader vertexArray
