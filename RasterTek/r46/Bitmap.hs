module Bitmap (Bitmap,Bitmap.initialize,bitmapTexture,bitmapGlowMap) where
...
data Bitmap = Bitmap {
    ...
    getGlowMap :: Maybe Texture,
    getWindowSize :: (GLsizei, GLsizei),
    getBitmapSize :: (GLsizei, GLsizei),
    getPrevPos :: (GLint, GLint) }
    deriving (Eq, Show)

initialize wWidth wHeight bmWidth bmHeight texFile texUnit glowFile glowUnit = do
    ...
    (success1,texture) <- Texture.initialize texFile texUnit True
    (success2,glowmap) <- Texture.initialize glowFile glowUnit False
    
    glBindVertexArray 0
    
    return (success1 && success2, Just $ Bitmap vertexArray vertexBuffer indexBuffer 4 6 texture glowmap (wWidth,wHeight) (bmWidth,bmHeight) (-1,-1))
...
instance Shutdown Bitmap where
    shutdown (Bitmap vArray vBuffer iBuffer _ _ (Just texture) (Just glowmap) _ _ _) = do
        shutdown texture
        shutdown glowmap
...
bitmapGlowMap = maybe 0 textureUnit . getGlowMap
