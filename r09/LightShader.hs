...
initialize = do
    ...
    then return (False, Nothing)
        ...
        withArray0 0 (map castCharToCChar "position") $ \ptr ->
            glBindAttribLocation program 0 ptr
        withArray0 0 (map castCharToCChar "texcoord") $ \ptr ->
            glBindAttribLocation program 1 ptr
        withArray0 0 (map castCharToCChar "normal") $ \ptr ->
        ...
        let success = all (/= -1) [world,view,projection,tex0,direction,diffuse,ambient]
        return . (,) success . Just $ LightShader
            program vShader fShader
            world view projection
            tex0
            direction diffuse ambient
...
parameters (LightShader program _ _ world view projection texture direction diffuse ambient)
    worldMx viewMx projectionMx texUnit lightDir diffuseClr ambientClr = do
