module RenderTexture (RenderTexture,initialize,setRenderTarget,textureUnit) where
...
initialize width height texUnit = do
    ...
    then return (True, Just $ RenderTexture (width,height) frameBuffer depthBuffer rendertex texUnit)
    else do
