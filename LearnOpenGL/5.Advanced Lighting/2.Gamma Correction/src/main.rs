...
fn main() {
    ...
    let shader = Program::new(Rc::clone(&gl)).link("src/2.gamma_correction.vs", "src/2.gamma_correction.fs");

    let plane = VertexArray::new_shape(Rc::clone(&gl), Vec::from(&PLANE_VERTICES[..]), Vec::from(&INDICES[..6]));

    let floor = Texture::new(Rc::clone(&gl), "", "").load("resources/textures/wood.png", false);
    let floor_gamma = Texture::new(Rc::clone(&gl), "", "").load("resources/textures/wood.png", true);

    shader.apply();
    shader.set_int("floorTexture", 0);

    let light_positions = vec![Vec3::new(-3.0, 0.0, 0.0), Vec3::new(-1.0, 0.0, 0.0), Vec3::new(1.0, 0.0, 0.0), Vec3::new(3.0, 0.0, 0.0)];
    ...
    let mut gamma = GammaCorrect { is_on: false, pressed: false };
    ...
    while !window.should_close() {
        ...
        process_input(&mut camera, &mut gamma, &mut window, delta_time);
        ...
        shader.set_vec_vec3("lightPositions", &light_positions);
        shader.set_vec_vec3("lightColors", &light_colors);
        shader.set_vec3("viewPos", camera.position());
        shader.set_int("gamma", gamma.is_on as i32);

        plane.bind();
        if gamma.is_on {
            floor_gamma.bind_active(gl::TEXTURE0);
        } else {
            floor.bind_active(gl::TEXTURE0);
        }
        plane.draw();

        println!("{}", if gamma.is_on { "Gamma enabled" } else { "Gamma disabled" });
...
fn process_input(camera: &mut Camera, gamma: &mut GammaCorrect, window: &mut Window, delta_time: f32) {
    ...
    if window.get_key(Key::Space) == Action::Press && !gamma.pressed {
        gamma.is_on = !gamma.is_on;
        gamma.pressed = true;
    }
