...
initialize window width height = Input window (width,height) (0,0) False False False False False False False False False
...
instance Render Input where
    render input = do
        ...
        mouseleft <- getMouseButton (getWindow input) MouseButton'1
        ...
        return . (,) True $ input {
            getMousePos = mouse,
            getLeftClicked = mouseleft == MouseButtonState'Pressed,
