...
initialize = do
    ...
    then return (False, Nothing)
        ...
        depth <- withArray0 0 (map castCharToCChar "depth") $ glGetUniformLocation program
        ...
        let success = all (/= -1) [world,view,projection,lightview,lightproject,lightpos,texture,depth,ambient,diffuse]
        return (success, Just $
            ShadowShader program shaders world view projection lightview lightproject lightpos texture depth ambient diffuse)
...
parameters (ShadowShader program _ world view projection lightview lightproject lightpos texture depth ambient diffuse)
    worldMx viewMx projectionMx lightviewMx lightprojectMx position texunit depthunit ambientClr diffuseClr = do
