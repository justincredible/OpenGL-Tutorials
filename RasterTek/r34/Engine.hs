...
run = do
    ...
    window <- openWindow "Tutorial 34" width height
    ...
    let input = Input.initialize window screenW screenH
        position = Position.initialize { getPosition = [0,1.5,-11] }
    ...
    loop engine = do
        ...
        let position = turn (leftDown input) (rightDown input) (getCameraPos engine) { getFrameTime = getTiming timer }
        
        (success, frame) <- update (getFrame engine) (LF (getPosition position)) >>= render . snd
