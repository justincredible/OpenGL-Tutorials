...
initialize = do
    ...
    then return (False, Nothing)
        ...
        let success = all (/= -1) [world,view,projection,viewpnt,projectpnt,texture,projectex,lightpos,ambient,diffuse]
        return (success, Just $
            ProjectionShader program shaders world view projection viewpnt projectpnt lightpos texture projectex ambient diffuse)
...
parameters (ProjectionShader program _ world view projection viewpnt projectpnt lightpos texture projectex ambient diffuse)
    worldMx viewMx projectionMx viewPntMx projectPntMx position texunit projectexunit ambientClr diffuseClr = do
