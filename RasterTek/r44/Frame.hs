...
initialize window width height = do
    ...
    light <- PointLight.initialize [2,5,-2] [1,1,1,1]
    ...
    (texscs,projectex) <- Texture.initialize "asset/grate.tga" 3 True
    ...
    return (and [cubescs,groundscs,texscs,shaderscs], Just $
        Frame window opengl camera {
                ...
                Camera.getPosition = [0,7,-10],
                getRotation = [7*pi/36,0,0] }
            light viewpoint cube ground projectex shader)
...
instance Render Frame where
    render frame = do
        ...
        parameters shader
            ...
            (PointLight.getPosition . getLight $ frame)
            (modelTexture floor)
