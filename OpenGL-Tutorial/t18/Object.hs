...
computeTangentBasis [positions, coordinates, normals] = process positions coordinates normals id id
    ...
    process (v0:v1:v2:vs) (u0:u1:u2:us) (n0:n1:n2:ns) dt db = let
        ...
        t0 = fixhand (dot (cross n0 t) b) . normalise . minus t $ map (*dot n0 t) n0
        t1 = fixhand (dot (cross n1 t) b) . normalise . minus t $ map (*dot n1 t) n1
        t2 = fixhand (dot (cross n2 t) b) . normalise . minus t $ map (*dot n2 t) n2
        in
