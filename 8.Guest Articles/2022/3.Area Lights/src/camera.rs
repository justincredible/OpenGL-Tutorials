pub mod camera {
    ...
    const _YAW: f32 = -PI / 2.0;
    const _PITCH: f32 = 0.0;
    const SPEED: f32 = 2.5;
    ...
    impl Camera {
        pub fn new(position: Vec3, yaw: f32, pitch: f32, last_x: f32, last_y: f32) -> Self {
            let world_up = Vec3::Y;
        ...
        fn _new_spec(pos_x: f32, pos_y: f32, pos_z: f32, up_x: f32, up_y: f32, up_z: f32, yaw: f32, pitch: f32) -> Self {
            let world_up = Vec3::new(up_x, up_y, up_z).normalize();
        ...
        fn calculate_vectors(&mut self) {
            self.front = Vec3::new(self.yaw.cos() * self.pitch.cos(), self.pitch.sin(), self.yaw.sin() * self.pitch.cos()).normalize();
