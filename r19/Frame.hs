...
data Frame = Frame {
    ...
    getShader :: Maybe AlphaMapShader }
    deriving (Eq, Show)

initialize window width height = do
    opengl <- OpenGL.initialize window width height
    camera <- fmap snd $ Camera.initialize >>= render
    (modelscs,model) <- Model.initialize "asset/square.txt"
        "asset/stone01.tga" "asset/dirt01.tga"
        "asset/alpha01.tga" 0 False
    (shaderscs,shader) <- AlphaMapShader.initialize
